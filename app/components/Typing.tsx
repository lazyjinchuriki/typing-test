    ...
    const handleKeyPress = (e: KeyboardEvent) => {
        ...
        if (e.key === 'Backspace') {
            if (newTypedWords[state.wordIndex]) {
                newTypedWords[state.wordIndex] = newTypedWords[state.wordIndex].slice(0, -1);
            }
        } else {
            if (newTypedWords[state.wordIndex]) {
                ...
                <div
                    ...
                    <div className='inline-flex items-center justify-center transform translate-x-1/2'>
                        {paragraph.split(" ").map((word, wordIndex) => {
                            return (
                                <span key={wordIndex} className="m-1">
                                    {word.split("").map((letter, letterIndex) => {
                                        return (
                                            <>
                                                ...
                                                <span key={letterIndex} className={`text-3xl font-light ${wordIndex === state.wordIndex && letterIndex === state.letterIndex ? "text-blue-700" : wordIndex < state.wordIndex ? (word === state.typedWords[wordIndex] ? "text-green-500" : "text-red-500") : "text-black"}`}>{letter}</span>
                                            </>
                                        );
                                    })}
                                </span>
